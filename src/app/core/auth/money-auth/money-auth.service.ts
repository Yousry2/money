...
export class MoneyAuthService implements AuthService {
  ...
  public setSession(authResult): void {
    ...
    const expiresAt = 1 * 60 * 60 * 1000 + new Date().getTime();
  ...
  public isAuthenticated(): boolean {
    const expiresAt = JSON.parse(localStorage.getItem(MoneyAuthService.EXPIRES_AT));
    
    // check if the current time didn't exceed the sexxion expiry time
    if (localStorage.getItem(MoneyAuthService.ACCESS_TOKEN) != null && new Date().getTime() < expiresAt)
      return true;
  }
